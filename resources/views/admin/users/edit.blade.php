        ...
        <div class="max-w-lg mx-auto px-4">
            <div class="bg-white rounded-2xl border border-gray-100 p-6">
                ...
                <form method="POST" action="{{ route('admin.users.update', $user) }}">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm text-gray-600 mb-1">Password <span class="text-gray-400 font-normal">(kosongkan jika tidak ingin mengubah)</span></label>
                            ...
                            @foreach($events as $event)
                                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50">
                                    <input type="checkbox" name="event_ids[]" value="{{ $event->id }}"
                                        {{ in_array($event->id, old('event_ids', $assignedEventIds)) ? 'checked' : '' }}
